...
from common import get_list_of_py, kill_proc_tree, get_full_info, start_program, update_repo
from CONFIG import CONFIG


def block_access(update: Update, context: CallbackContext):
    if update.effective_user.id not in CONFIG.ADMINS:
        update.effective_message.reply_html(f"""
...
def restart(update: Update, context: CallbackContext):
    if context.args:
        status_msg = update.effective_message.reply_html("Trying to restart the process...")
        ...
        if process:
            ...
            try:
                ...
                status_msg = status_msg.edit_text(f"{status_msg.text_html}"
                ...
                status_msg.edit_text(f"{status_msg.text_html}"
                                     f"\nStarted the bot",
                                     parse_mode="HTML")
            except psutil.NoSuchProcess:
                update.effective_message.reply_html("Looks like someone already killed it!")
    else:
        update.effective_message.reply_html("<b>Format:</b> /restart alias"
